...
export class HeatClassifier {
     ...
     */
    calculateItemHeat(itemId, prices, quantities) {
        if (prices.length === 0) {
            return {
                itemId,
                heatScore: 0,
        ...
        const heatScore = (volatility * this.volatilityWeight) +
     ...
     */
    getBlockAverageHeat(heatScores) {
        if (heatScores.size === 0)
            return 0;
        let sum = 0;
     ...
     */
    calculateChangeFrequency(prices) {
        if (prices.length < 2)
            return 0;
        let changes = 0;
...
//# sourceMappingURL=HeatClassifier.js.map